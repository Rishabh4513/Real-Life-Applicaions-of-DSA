//Hello world of phaser = basic game = single scene in spin and win game
//How to create a basic skeleton of the game -> Game loop


let prizes_config = {
    count: 12,
    prize_names: ["3000 Credits", "35% Off", "Hard Luck", "70% OFF", "Swagpack", "100% OFF", "Netflix", "50% Off", "Amazon Voucher", "2 Extra Spin", "CB Tshirt", "CB Book"]
}


let config = {
    type: Phaser.CANVAS,
    width: 800,
    height: 600,

    scene: {
        preload: preload,
        create: create,
        update: update,

    },
    audio: {
        disableWebAudio: true
    }

};

let game = new Phaser.Game(config);

function preload() {
    console.log("preload");

    this.load.image('background', 'Assets/back.jpg');
    console.log(this);
    this.load.image('wheel', 'Assets/wheel.png');
    this.load.image('pin', 'Assets/pin.png');
    this.load.image('stand', 'Assets/stand.png');
    this.load.audio('audio_beam', ['Assets/Mario.mp3']);
}

function create() {
    console.log("Create");

    let W = game.config.width;
    let H = game.config.height;
    let background = this.add.sprite(0, 0, 'background');
    background.setPosition(W / 2, H / 2);
    background.setScale(0.20);

    //lets create the stand
    let stand = this.add.sprite(W / 2, H / 2 + 250, "stand");
    stand.setScale(0.25);

    //lets create the pin
    let pin = this.add.sprite(W / 2, H / 2 - 250, "pin");
    pin.setScale(0.25);
    pin.depth = 1;

    //lets create the wheel
    this.wheel = this.add.sprite(W / 2, H / 2, "wheel");
    this.wheel.setScale(0.25);

    //adding event listener
    this.input.on("pointerdown", spinwheel, this);

    //lets create a text object
    font_style = {
        font: "bold 30px Arial",
        alinment: "center",
        color: "red",
    }
    this.game_text = this.add.text(10, 10, "Welcome to spin and win", font_style);

    var music = this.sound.add('audio_beam');
    music.play();

}
//game loop
function update() {
    console.log("Update");

    //this.wheel.angle+=1;
}

function spinwheel() {
    console.log("mouse is pressed");
    //this.game_text.setText("You clicked");

    let rounds = Phaser.Math.Between(2, 4);
    //console.log(rounds);
    let degrees = Phaser.Math.Between(0, 11) * 30;

    let total_angle = rounds * 360 + degrees;
    console.log(total_angle);

    let idx = prizes_config.count - 1 - Math.floor(degrees / (360 / prizes_config.count));

    tween = this.tweens.add({
        targets: this.wheel,
        angle: total_angle,
        ease: "Cubic.easeOut",
        //delay:2000,
        duration: 6000,
        callbackScope: this,
        onComplete: function() {
            this.game_text.setText("Congratulations!!! You won " + prizes_config.prize_names[idx]);
        }
    });
}